...
void Zoom::zoom(CameraData& data, float d) const {
  ...
  XMVECTOR camera_z_axis = XMVector4Normalize(XMVector4Transform(i_z, look_at));
  XMVECTOR z_translation = XMVectorScale(camera_z_axis, factor);
  XMMATRIX translation = XMMatrixTranspose(XMMatrixTranslationFromVector(z_translation));
