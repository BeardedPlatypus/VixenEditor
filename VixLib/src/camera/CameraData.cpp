...
CameraData::CameraData(XMFLOAT4 eye,
  ...
  this->recalculateLookAtMatrix();
  this->recalculatePerspectiveMatrix();
}


void CameraData::recalculateLookAtMatrix() {
  ...
  XMStoreFloat4x4(&(this->look_at_matrix),
                  ...
                  XMMatrixLookAtLH(v_eye, v_center, v_up));

  XMStoreFloat(&(this->zoom), XMVector4Length(v_eye - v_center));
}

void CameraData::recalculatePerspectiveMatrix() {
  DirectX::XMMATRIX perspective = DirectX::XMMatrixPerspectiveFovLH(this->fov_angle_y,
                                                                    this->aspect_ratio,
                                                                    this->z_near,
                                                                    this->z_far);
  ...
  DirectX::XMStoreFloat4x4(&(this->projection_matrix), perspective);// *orientation );
}
